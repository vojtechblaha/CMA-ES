...
class TransferModel:
    ...
    # --------------------
    def predict(self, X):
        self.best_idx = np.argmax(self.models_dist)#int(np.random.choice(len(self.models), p=self.models_dist))
    ...
    # --------------------
    def update(self, X_obs, y_obs, L_dict):
        if len(X_obs) < 20:
            return
        ...
        for i in range(len(self.models)):
            ...
            self.models[i].fit(X_obs[:-10], y_obs[:-10])
            y_pred = self.models[i].predict(X_obs[-10:])
            r_pred = np.argsort(y_pred)
            y_true = y_obs[-10:]
            r_true = np.argsort(y_true)
            
            dist = self.normalized_kendall_distance(r_true, r_pred)
            self.accs[i].append(1-dist)
    ...
    def optimize(self, problem, args):
        ...
        while not es.stop() and n_real_evals < max_evals:
            ...
            if L_dict is not None:
                model.update(self.x_obs, self.y_obs, L_dict)
